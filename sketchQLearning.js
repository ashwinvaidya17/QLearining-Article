...
async function train() {
	//train loop
	for (episodes = 0; episodes < total_episodes; episodes++) {
		resetEnv();
		gameOver = false;

		for (var s = 0; s < max_steps; s++) {
			var exp_exp_tradeoff = Math.random();

			if (exp_exp_tradeoff < epsilon)
				action = Math.floor(Math.random() * 4); //explore
			else
				action = argMax(qtable[state]); //exploit

			await sleep(500); // to slow down the steps for visualization
			redraw();
